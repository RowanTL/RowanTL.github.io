...
To begin this proof by contraposition, suppose $$G$$ is a directed graph with no root nodes and sink nodes.
Let $$v \in V$$ represent a node of $$G$$. Because $$v$$ cannot be a root node, there must exist
...
By proving the contrapositive of the original conditional, the statement: if $$G$$ is a DAG,
